...
void main() {
  ...
  // Aurora Sway: Slow, sweeping time-based distortion
  float sway = sin(iTime * 0.15) * 0.2;
  
  vec2 rayRefDir1 = normalize(vec2(cos(0.785398 + halfSpread + sway), sin(0.785398 + halfSpread + sway)));
  vec2 rayRefDir2 = normalize(vec2(cos(0.785398 - halfSpread - (sway * 0.7)), sin(0.785398 - halfSpread - (sway * 0.7))));
  ...
  // Aurora Breath: Subtle intensity pulsing
  float breathe = 1.0 + 0.15 * sin(iTime * 0.4);
  float brightness = (iIntensity * breathe) * 0.4 / pow(max(distanceToLight, 0.001), iFalloff);
